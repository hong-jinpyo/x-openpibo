...
메인 컨트롤러로 메시지 (``#code:data!`` 포멧) 가 전달됩니다.  
이후 메인컨트롤러는 메시지를 파악하여 주어진 기능을 수행합니다.

다음 메시지 상세 설명은 기능 이름에 대해 code, data, get, set으로 설명됩니다.
**code** 와 **data** 는 메인 컨트롤러로 전송되는 메시지이며, **get** 은 메시지 전달 후 반환값, **set** 은 메시지 전달 후 수행되는 디바이스 설정을 의미합니다.
...
:메시지 상세 설명:
  ...
  * NEOPIXEL_OFFSET_SET
    ...
    만약 오프셋이 '255,255,255,255,255,0' 이면, 네오픽셀을 흰 색으로 설정해도 오른쪽 눈에서 파란 빛이 나오지 않습니다.
  ...
  * SYSTEM
    ...
    * get: 아래의 정보가 모두 출력됩니다.
      ...
      * 버튼 감지:
        ...
        * ``on`` : 전원버튼 누름 감지 시 출력됩니다.
        * ``-`` : 전원버튼 누름 감지가 안되면 출력됩니다.
...
class Device:
  ...
  def locked(self):
    """
    Device가 사용 중인지 확인합니다. Device가 사용중일 때 메시지를 보내지 않도록 주의합니다.
  ...
  def send_cmd(self, code, data:str=""):
    """
    Device에 메시지 코드/데이터를 전송합니다. 입력된 메시지는 ``#code:data!`` 의 포멧으로 변경되어 전달됩니다.
  ...
  def send_raw(self, raw):
    ...
    :param str raw: 실제 전달되는 메시지
      ...
      Device에 전송되는 메시지의 포멧은 ``#code:data!`` 입니다.
